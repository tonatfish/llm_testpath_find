...
def archive_language_setting(driver: WebDriver, language_index: int):
    ...
    time.sleep(1)
    ShellUtils.tap_element(driver, elements["publicSelect"])
    time.sleep(0.5)
...
# unable to test because some unexpected file may appear in download folder
@pytest.mark.parametrize("language_index", [1, 2, ])
def test_public_archive(function_archive_driver: WebDriver, tmp_path: Path, language_index: int):
    time.sleep(1)
    archive_language_setting(function_archive_driver, language_index)

    ShellUtils.tap_element(function_archive_driver, elements["publicSelect"])
    time.sleep(1)
    function_archive_driver.save_screenshot(str(tmp_path / "archive_public_result.png"))
    result = ImageUtils.crop_element_with_path(
        str(tmp_path/"archive_public_result.png"), elements["archiveSectionNoTime"])
    # result.save("samples/archive/archivePublic_language" + str(language_index) + ".png")
    assert ImageUtils.compare_images(
        "samples/archive/archivePublic_language" + str(language_index) + ".png", result, 0.99)