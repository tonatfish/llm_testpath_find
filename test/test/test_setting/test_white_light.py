...
def test_white_light(function_record_driver: WebDriver, tmp_path: Path, audioName: str):
    ...
    with open(str(tmp_path / "white_light.mp4"), 'wb') as record_video:
    ...
    frame_count = 0
    time_ctrl = 0
    vc = cv2.VideoCapture(str(tmp_path / "white_light.mp4"))
    assert vc.isOpened()
    ...
    while rval and frame_count < screenShotCount:
        ...
        if (time_ctrl % frame_interval == 0):
            cv2.imwrite(str(tmp_path / ("audio_white_light_" + audioName +
    ...
    for i in range(frame_count):
        ...
        result = ImageUtils.crop_element_with_path(str(
            tmp_path / ("audio_white_light_" + audioName + "_result_" + str(i) + ".png")), elements_mainwindow["surfaceView_upperBorder"])
        # result.save(str("samples/tmp/" + ("audio_white_light_" + audioName + "_result_" + str(i) + ".png")))
        if ImageUtils.test_color_in_image_from_image(result, (255, 255, 255)):
            whiteDetected += 1
    print(whiteDetected)
    print(frame_count)
    if whiteDetected == 0:
        result_list = []
        for i in range(frame_count):
            result_list.append(ImageUtils.mask_elements_in_image(tmp_path / ("audio_white_light_" + audioName + "_result_" + str(i) + ".png"), elements_mainwindow["tvDateTime"]))
        ImageUtils.save_and_report_result_with_video(result_list, video_bytes)
    assert whiteDetected > 0
